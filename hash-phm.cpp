...
#include <pthread.h>
#include <iomanip>
#include "phashmap.h" //Kevin's implementation of persistent hash table
using namespace std;
...
*/
int HB_insert(phashmap map, Package &package) {
	//int opt = package.operation();//opt not be used?
	string package_str = package.SerializeAsString();
	//int ret = db.set(package.virtualpath(), package_str); //virtualpath as key
...
string HB_lookup(phashmap map, Package &package) {
	string value;
...
int HB_remove(phashmap map, Package &package) {
	string key = package.virtualpath();
...
//					cout << "send back result to host: " << dest.host << "; port: " << dest.port << endl;
					int sock = 0;
...
*/
	...
	pmap = phashmap(fileName, 10000000, 100000, 0.7);
