...
//	cout << "Usage: ./client <num_operations> <memberList> <configFile>"<< endl;
	//	For BGP
	 ...
	 int pid = getpid();
	 unsigned int v = myhash(torusID.c_str(), 10000000) + pid + getTime_msec();
...
//cout<<"start to insert..."<<endl;
	...
	if (testClient.initialize(cfgFile, memberList, TCP) != 0) {
                cout << "Crap! ZHTClient initialization failed, program exits." << endl;
                exit(-1);
        }

//	cout <<"client cache size: "<< testClient.CACHE_SIZE <<endl;	
