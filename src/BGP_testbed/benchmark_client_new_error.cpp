...
int benmarkInsert_array(string cfgFile, string memberList,
		vector<char*> &pkgList, ZHTClient &clientRet, int numTest,
		int lenString) {
	...
	for (int i = 0; i < numTest; i++) {
		...
		char pkgStrArray[1024];
		memset(pkgStrArray, '\0', 1024);
		//string str = package.SerializeAsString();
		package.SerializeToArray(pkgStrArray, 1023);
...
float benchmarkLookup_array(vector<char*> strList, ZHTClient &client) {
	vector<char*>::iterator it;
...
//		cout <<"Lookup: "<< (*it).c_str() << endl;
		...
		string keyPkg(key);
		if (client.lookup(keyPkg, result) < 0) {
			errCount++;
		} else if (result.empty()) { //empty string
			errCount++;
		}
		cout << "Client: What I get from key: " << keyPkg << endl;
		;
...
//		cout << "lookup result = "<<result.c_str() << endl;
	}
...
vector<char*> pkgVector;

int initZhtClient(const string& cfgFile, const string& memberList,
		ZHTClient &clientRet);

void buildPkgArray(const int& numTest, const int& lenString);

int benmarkInsertOnArray(const int& numTest, ZHTClient &clientRet,
		char **pPkgArray, const int& length);

float benchmarkLookupOnArray(ZHTClient &client, char **pPkgArray,
		const int& length);

float benchmarkRemoveOnArray(ZHTClient &client, char **pPkgArray,
		const int& length);
...
//	cout << "Usage: ./client <num_operations> <memberList> <configFile>"<< endl;
	//	For BG/P
	 ...
	 int pid = getpid();
	 rSeed = myhash(torusID.c_str(), 1000000) + pid;
...
//cout<<"start to insert..."<<endl;
	...
	int status = initZhtClient(cfgFile, memberList, testClient);

	if (status != 0)
		exit(1);

	buildPkgArray(numOper, 15);

//	benchmarkInsert(cfgFile, memberList, pkgList, testClient, numOper, 15); //25fro 128bytes.
//	benmarkInsert_array(cfgFile, memberList, pkgList, testClient, numOper, 15);
	benmarkInsertOnArray(numOper, testClient, &pkgVector[0], pkgVector.size());

//cout << "Client:main, start lookup \n";
//	benchmarkLookup(pkgList, testClient);
//	benchmarkLookup_array(pkgList, testClient);
	benchmarkLookupOnArray(testClient, &pkgVector[0], pkgVector.size());

//	benchmarkRemove(pkgList, testClient);
	benchmarkRemoveOnArray(testClient, &pkgVector[0], pkgVector.size());
...
void buildPkgArray(const int& numTest, const int& lenString) {
	...
	for (int i = 0; i < numTest; i++) {
		...
		strcpy(pc, str.c_str());
		pkgVector.push_back(pc);

#if ILOG
		cout << "in buildPkgArray" << pc << endl;
#endif
	}
}
...
#if ILOG
		cout << "in benmarkInsertOnArray" << endl << pPkgArray[i] << endl;
#endif

		string sPkg(pPkgArray[i]);

		int ret = clientRet.insert(sPkg);
...
float benchmarkLookupOnArray(ZHTClient &client, char **pPkgArray,
	...
	cout << "Lookup " << length - errCount << " packages out of " << length
			<< ", cost " << end - start << " ms" << endl;
...
float benchmarkRemoveOnArray(ZHTClient &client, char **pPkgArray,
	...
	/*	for (int i = 0; i < length; i++) {
	 ...
	 Package package;
	 package.ParseFromString((pPkgArray[i]));
	 package.set_operation(2); //1 for look up, 2 for remove, 3 for insert
	 package.set_replicano(5); //3 not original, 5: original

	 pPkgArray[i] = (char*) package.SerializeAsString().c_str();
	 }*/
	...
	cout << "Remove " << length - errCount << " packages out of " << length
			<< ", cost " << end - start << " ms" << endl;
