...
int32_t HB_insert(NoVoHT *map, Package &package) {
	...
	/*
	 char valArray[MAX_MSG_SIZE];
	 memset(valArray, '\0', MAX_MSG_SIZE);
	 package.SerializeToArray(valArray, MAX_MSG_SIZE);
	 */

	char *valArray = new char[package_str.size() + 1];
	strcpy(valArray, package_str.c_str());
...
#if ILOG
	cout << "package.virtualpath().c_str()=" << package.virtualpath().c_str()
	<< endl;
	cout << "serialized string:" << valArray << endl;
#endif

//	strcpy(key, package.virtualpath().c_str());
	string keyStr = package.virtualpath();
//	char * key = new char[keyStr.length()+1];

	/*	char* key = (char*) malloc(keyStr.length() * sizeof(char));
	 strcpy(key, package.virtualpath().c_str());*/

	char *key = new char[keyStr.size() + 1];
	std::strcpy(key, keyStr.c_str());

	//	        memset(key, '\0', 1024);
	//      cout << "package.virtualpath().c_str()="<<package.virtualpath().c_str()<<endl;
	//memcpy(key, keyStr.c_str(), keyStr.length()); //string here.
	//strcpy(key, package.virtualpath().c_str());
	//int ret = db.set(package.virtualpath(), package_str); //virtualpath as key

//	cout << "To insert to pmap...value = " << value << endl<< endl;
...
//      cout<<"Insert: k-v ready. put..."<<endl;
	...
	int ret = map->put(key, valArray);
//	cout <<"Just after insert, checkout result value = "<<map->get(key)<<endl<< endl;
...
//	cout << "lookup result = " << result << endl<< endl;
...
//      free(value);
	...
	ret = chmap.insert(MY_PAIR(key, valArray));
...
//			cout << "Lookup...3" << endl;
	...
	case 3: {
		...
		if (package.virtualpath().empty()) {
			...
			//operation_status = HB_insert(pmap, package);HB_insert_cstr_
