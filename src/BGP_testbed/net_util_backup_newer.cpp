...
int makeSvrSocket(int port, bool tcp) { //only for svr
	...
	if (svrSock < 0) {
		 cerr << "Error occurred when creating the socket: "<< svrSock << ",  to the server port: "<< port << strerror(errno)<<endl;
	...
	if (tcp == true) {
		...
		if (bind(svrSock, (struct sockaddr *) &svrAdd_in,
			 ...
			 cerr << "Error occurred binding the socket: "<< svrSock << ",  to the server port: "<< port << strerror(errno)<<endl;
...
//		svrAdd_in.sin_port = htons(port);
	...
	if (tcp == true) { //TCP needs listen, UDP does not.
		...
		if (listen(svrSock, 8000) < 0) {
			cerr << "Error occurred while enabling listen on the socket: "<< svrSock << ", "<< strerror(errno)<<endl;
			close(svrSock);
			return -1;
		}
	}
...
int svr_accept(int sock, bool tcp) {
	...
	if (tcp == true) {
		...
		consocket = accept(sock, (struct sockaddr *) &dest,
				(socklen_t *) &socksize);
		if (consocket < 0) {
			cerr << "Error occurred while accepting on the socket: "<< sock << ", "<< strerror(errno)<<endl;
		}
		return consocket;
	} else {
		return sock;
...
int makeClientSocket(const char* host, int port, bool tcp) {
	...
	if (tcp == true) {
		...
		if (to_sock < 0) {
			cerr<<"Error occurred when creating the socket:"<< to_sock << ", "<<strerror(errno)<<endl;
			close(to_sock);
			return -1;
		}
	}
